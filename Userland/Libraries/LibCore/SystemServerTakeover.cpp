...
static void parse_sockets_from_system_server()
{
    ...
    for (auto const socket : StringView { sockets, strlen(sockets) }.split_view(';')) {
        ...
        s_overtaken_sockets.set(params[0].to_string(), params[1].to_number<int>().value());
    }
...
ErrorOr<NonnullOwnPtr<Core::LocalSocket>> take_over_socket_from_system_server(String const& socket_path)
{
