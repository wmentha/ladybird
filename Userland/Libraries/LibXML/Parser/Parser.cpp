...
ErrorOr<void, ParseError> Parser::parse_internal()
{
    ...
    if (auto it = find_if(matched_source.begin(), matched_source.end(), s_restricted_characters); !it.is_end()) {
        return parse_error(
            ...
            MUST(String::formatted("Invalid character #{:x} used in document", *it)));
    }

    if (!m_lexer.is_eof())
        return parse_error(m_lexer.current_position(), String { "Garbage after document"_string });
...
ErrorOr<void, ParseError> Parser::expect(StringView expected)
{
    ...
    if (!m_lexer.consume_specific(expected)) {
        if (m_options.treat_errors_as_fatal)
            return parse_error(m_lexer.current_position(), MUST(String::formatted("Expected '{}'", expected)));
    }
...
ErrorOr<void, ParseError> Parser::parse_version_info()
{
    ...
    if (version_string == "1.0") {
        ...
        if (version_string != "1.1" && m_options.treat_errors_as_fatal)
            return parse_error(m_lexer.current_position(), MUST(String::formatted("Expected '1.1', found '{}'", version_string)));
    }
...
ErrorOr<Name, ParseError> Parser::parse_processing_instruction_target()
{
    ...
    if (target.equals_ignoring_ascii_case("xml"sv) && m_options.treat_errors_as_fatal) {
        return parse_error(
            ...
            String { "Use of the reserved 'xml' name for processing instruction target name is disallowed"_string });
    }
            ...
            if (m_options.resolve_external_resource) {
                ...
                if (!resource_result.is_error()) {
                    auto declarations = TRY(resource_result.release_value().visit(
                        [&](String resolved_source) -> ErrorOr<Vector<MarkupDeclaration>, ParseError> {
                            ...
                            if (!m_lexer.is_eof()) {
                                return parse_error(
                                    ...
                                    MUST(String::formatted("Failed to resolve external subset '{}': garbage after declarations", doctype.external_id->system_id.system_literal)));
                            }
                            return declarations;
                        },
...
ErrorOr<void, ParseError> Parser::parse_element()
{
    ...
    if (m_options.treat_errors_as_fatal && closing_name != tag.name)
        return parse_error(m_lexer.position_for(tag_location), String { "Invalid closing tag"_string });
...
ErrorOr<String, ParseError> Parser::parse_attribute_value_inner(StringView disallow)
{
    ...
    while (true) {
        ...
        if (m_lexer.next_is('<')) {
            ...
            return parse_error(m_lexer.current_position(), String { "Unescaped '<' not allowed in attribute values"_string });
        }
...
ErrorOr<Variant<Parser::EntityReference, String>, ParseError> Parser::parse_reference()
{
    ...
    if (name_result.is_error()) {
        ...
        if (!code_point.has_value() || !s_characters.contains(*code_point))
            return parse_error(m_lexer.position_for(reference_start), String { "Invalid character reference"_string });
...
ErrorOr<String, ParseError> Parser::resolve_reference(EntityReference const& reference, ReferencePlacement placement)
{
    ...
    if (reference_lookup.contains(reference.name))
        return parse_error(m_lexer.current_position(), MUST(String::formatted("Invalid recursive definition for '{}'", reference.name)));
            ...
            TRY(ge_declaration->definition.visit(
                ...
                [&](EntityDefinition const& definition) -> ErrorOr<void, ParseError> {
                    if (placement == ReferencePlacement::AttributeValue)
                        return parse_error(m_lexer.current_position(), MUST(String::formatted("Attribute references external entity '{}'", reference.name)));

                    if (definition.notation.has_value())
                        return parse_error(m_lexer.position_for(0), MUST(String::formatted("Entity reference to unparsed entity '{}'", reference.name)));

                    if (!m_options.resolve_external_resource)
                        return parse_error(m_lexer.position_for(0), MUST(String::formatted("Failed to resolve external entity '{}'", reference.name)));

                    auto result = m_options.resolve_external_resource(definition.id.system_id, definition.id.public_id);
                    if (result.is_error())
                        return parse_error(m_lexer.position_for(0), MUST(String::formatted("Failed to resolve external entity '{}': {}", reference.name, result.error())));

                    if (!result.value().has<String>())
                        return parse_error(m_lexer.position_for(0), MUST(String::formatted("Failed to resolve external entity '{}': Resource is of the wrong type", reference.name)));

                    resolved = result.release_value().get<String>();
                    return {};
                }));
            break;
        }
    ...
    if (!resolved.has_value()) {
        ...
        if (reference.name == "quot")
            return "\"";
        return parse_error(m_lexer.position_for(0), MUST(String::formatted("Reference to undeclared entity '{}'", reference.name)));
    }
