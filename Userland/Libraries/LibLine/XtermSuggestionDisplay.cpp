...
ErrorOr<void> XtermSuggestionDisplay::display(SuggestionManager const& manager)
{
    ...
    TRY(manager.for_each_suggestion([&](auto& suggestion, auto index) -> ErrorOr<IterationDecision> {
        ...
        if (spans_entire_line) {
            ...
            auto field = MUST(String::formatted("{: <{}}  {}", suggestion.text_string(), longest_suggestion_byte_length_without_trivia, suggestion.display_trivia_string()));
            TRY(stderr_stream->write_until_depleted(MUST(String::formatted("{: <{}}", field, longest_suggestion_byte_length + 2))));
            num_printed += longest_suggestion_length + 2;
        }
    ...
    if (m_pages.size() > 1) {
        ...
        auto string = MUST(String::formatted("{:c} page {} of {} {:c}", left_arrow, page_index + 1, m_pages.size(), right_arrow));
