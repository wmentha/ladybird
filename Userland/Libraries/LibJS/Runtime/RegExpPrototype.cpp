...
JS_DEFINE_NATIVE_FUNCTION(RegExpPrototype::symbol_replace)
{
    ...
    if (next_source_position >= string.length_in_code_units())
        return PrimitiveString::create(vm, TRY(accumulated_result.to_string()));
...
JS_DEFINE_NATIVE_FUNCTION(RegExpPrototype::symbol_split)
{
    ...
    auto new_flags = flags.find('y').has_value() ? move(flags) : MUST(String::formatted("{}y", flags));
...
JS_DEFINE_NATIVE_FUNCTION(RegExpPrototype::to_string)
{
    ...
    // 6. Return result.
    return PrimitiveString::create(vm, MUST(String::formatted("/{}/{}", pattern, flags)));
}
