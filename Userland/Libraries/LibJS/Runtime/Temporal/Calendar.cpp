...
ThrowCompletionOr<double> resolve_iso_month(VM& vm, Object const& fields)
{
    ...
    auto month_code_string = month_code.as_string();
...
ThrowCompletionOr<Object*> default_merge_calendar_fields(VM& vm, Object const& fields, Object const& additional_fields)
{
    ...
    for (auto& key : fields_keys) {
        ...
        if (!key.as_string().is_one_of(vm.names.month.as_string(), vm.names.monthCode.as_string())) {
    ...
    for (auto& key : additional_fields_keys) {
        ...
        // See comment above.
        additional_fields_keys_contains_month_or_month_code_property |= key.as_string() == vm.names.month.as_string() || key.as_string() == vm.names.monthCode.as_string();
    }
