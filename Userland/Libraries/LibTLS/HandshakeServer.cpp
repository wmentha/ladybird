        ...
        if (extension_type == ExtensionType::SERVER_NAME) {
            ...
            if (extension_length > 0) {
                ...
                if (buffer.size() - res < sni_name_length)
                    return (i8)Error::NeedMoreData;
                m_context.extensions.SNI = String::from_utf8_without_validation({ (char const*)buffer.offset_pointer(res), sni_name_length });
                res += sni_name_length;
                ...
                if (alpn_length && alpn_length <= extension_length - 2) {
                    ...
                    while (alpn_position < alpn_length) {
                        ...
                        if (alpn_size + alpn_position >= extension_length)
                            break;
                        String alpn_str = String::from_utf8_without_validation({ (char const*)alpn + alpn_position, alpn_length });
