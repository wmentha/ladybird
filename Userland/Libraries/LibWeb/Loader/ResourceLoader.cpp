...
RefPtr<Resource> ResourceLoader::load_resource(Resource::Type type, LoadRequest& request)
{
    ...
    load(
        ...
        JS::create_heap_function(m_heap, [=](String const& error, Optional<u32> status_code, Optional<String> const&, ReadonlyBytes data, HTTP::HeaderMap const& headers) {
...
static String sanitized_url_for_logging(URL::URL const& url)
{
...
static void store_response_cookies(Page& page, URL::URL const& url, String const& set_cookie_entry)
{
...
static HTTP::HeaderMap response_headers_for_file(StringView path, Optional<time_t> const& modified_time)
{
    ...
    if (modified_time.has_value()) {
        ...
        response_headers.set("Last-Modified"sv, datetime.to_string("%a, %d %b %Y %H:%M:%S GMT"sv, Core::DateTime::LocalTime::No));
    }
...
static bool should_block_request(LoadRequest const& request)
{
    ...
    if (is_port_blocked(url.port_or_default())) {
        log_failure(request, MUST(String::formatted("Port #{} is blocked", url.port_or_default())));
        return true;
    }
...
void ResourceLoader::load(LoadRequest& request, JS::Handle<SuccessCallback> success_callback, JS::Handle<ErrorCallback> error_callback, Optional<u32> timeout, JS::Handle<TimeoutCallback> timeout_callback)
{
    ...
    auto respond_directory_page = [](LoadRequest const& request, URL::URL const& url, JS::Handle<SuccessCallback> success_callback, JS::Handle<ErrorCallback> error_callback) {
        ...
        if (maybe_response.is_error()) {
            ...
            if (error_callback)
                error_callback->function()(MUST(String::formatted("{}", maybe_response.error()), 500u, {}, {}, {}));
            return;
        }
    ...
    if (url.scheme() == "about") {
        ...
        Platform::EventLoopPlugin::the().deferred_invoke(JS::create_heap_function(m_heap, [success_callback, response_headers = move(response_headers)] {
            success_callback->function()(""_string, response_headers, {}, {});
        }));
        return;
    }
    ...
    if (url.scheme() == "data") {
        ...
        HTTP::HeaderMap response_headers;
        response_headers.set("Content-Type", data_url.mime_type.serialized().to_string());
    ...
    if (url.scheme() == "resource") {
        ...
        if (resource.is_error()) {
            ...
            if (error_callback)
                error_callback->function()(MUST(String::formatted("{}", resource.error()), {}, {}, {}, {}));
            return;
        }
    ...
    if (url.scheme() == "file") {
        ...
        FileRequest file_request(URL::percent_decode(url.serialize_path()), [this, success_callback, error_callback, request, respond_directory_page](ErrorOr<i32> file_or_error) {
            ...
            if (file_or_error.is_error()) {
                ...
                if (error_callback)
                    error_callback->function()(MUST(String::formatted("{}", file_or_error.error()), {}, {}, {}, {}));
                return;
            }
            ...
            if (st_or_error.is_error()) {
                ...
                if (error_callback)
                    error_callback->function()(MUST(String::formatted("{}", st_or_error.error()), {}, {}, {}, {}));
                return;
            }
            ...
            if (maybe_file.is_error()) {
                ...
                if (error_callback)
                    error_callback->function()(MUST(String::formatted("{}", maybe_file.error()), {}, {}, {}, {}));
                return;
            }
            ...
            if (maybe_data.is_error()) {
                ...
                if (error_callback)
                    error_callback->function()(MUST(String::formatted("{}", maybe_data.error()), {}, {}, {}, {}));
                return;
            }
    ...
    if (url.scheme() == "http" || url.scheme() == "https") {
        ...
        auto on_buffered_request_finished = [this, success_callback, error_callback, request, &protocol_request = *protocol_request](auto, auto const& network_error, auto& response_headers, auto status_code, auto const& reason_phrase, ReadonlyBytes payload) mutable {
            ...
            if (network_error.has_value() || (status_code.has_value() && *status_code >= 400 && *status_code <= 599 && (payload.is_empty() || !request.is_main_resource()))) {
                ...
                if (error_callback)
                    error_callback->function()(error_builder.to_string(), status_code, reason_phrase, payload, response_headers);
                return;
            }
    ...
    auto not_implemented_error = MUST(String::formatted("Protocol not implemented: {}", url.scheme()));
...
RefPtr<Requests::Request> ResourceLoader::start_network_request(LoadRequest const& request)
{
    ...
    if (!headers.contains("User-Agent"))
        headers.set("User-Agent", m_user_agent.to_string());
