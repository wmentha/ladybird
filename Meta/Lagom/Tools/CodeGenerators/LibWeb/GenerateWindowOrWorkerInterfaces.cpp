...
static Optional<LegacyConstructor> const& lookup_legacy_constructor(IDL::Interface& interface)
{
    ...
    auto constructor_class = MUST(String::formatted("{}Constructor", name));
...
void add_@global_object_snake_name@_exposed_interfaces(JS::Object&);
    ...
    auto generated_header_path = LexicalPath(output_path).append(MUST(String::formatted("{}ExposedInterfaces.h", class_name))).string();
...
void add_@global_object_snake_name@_exposed_interfaces(JS::Object& global)
{
    ...
    auto add_interface = [](SourceGenerator& gen, StringView name, StringView prototype_class, Optional<LegacyConstructor> const& legacy_constructor, Optional<String> const& legacy_alias_name) {
        gen.set("interface_name", name);
        gen.set("prototype_class", prototype_class);
    ...
    auto generated_implementation_path = LexicalPath(output_path).append(MUST(String::formatted("{}ExposedInterfaces.cpp", class_name))).string();
...
ErrorOr<int> serenity_main(Main::Arguments arguments)
{
    ...
    for (String const& path : paths) {
        ...
        if (file_or_error.is_error()) {
            s_error_string = MUST(String::formatted("Unable to open file {}", path));
    ...
    for (size_t i = 0; i < paths.size(); ++i) {
        ...
        if (interface.name.is_empty()) {
            s_error_string = MUST(String::formatted("Interface for file {} missing", path));
            return Error::from_string_view(s_error_string.view());
        }
...
static ErrorOr<ExposedTo> parse_exposure_set(IDL::Interface& interface)
{
    ...
    if (!maybe_exposed.has_value()) {
        s_error_string = MUST(String::formatted("Interface {} is missing extended attribute Exposed"_string, interface.name));
        return Error::from_string_view(s_error_string.view());
    }
    ...
    if (exposed[0] == '(') {
        ...
        for (StringView candidate : exposed.substring_view(1, exposed.length() - 1).split_view(',')) {
            ...
            if (candidate == "Window"sv) {
                ...
                s_error_string = MUST(String::formatted("Unknown Exposed attribute candidate {} in {} in {}", candidate, exposed, interface.name));
                return Error::from_string_view(s_error_string.view());
            }
        }
        if (whom == ExposedTo::Nobody) {
            s_error_string = MUST(String::formatted("Unknown Exposed attribute {} in {}", exposed, interface.name));
            return Error::from_string_view(s_error_string.view());
        }
        return whom;
    }
