...
CppType idl_type_name_to_cpp_type(Type const& type, Interface const& interface)
{
    if (is_platform_object(type) || type.name() == "WindowProxy"sv)
        return { .name = MUST(String::formatted("JS::Handle<{}>", type.name())), .sequence_storage_type = SequenceStorageType::MarkedVector };

    if (is_javascript_builtin(type))
        return { .name = MUST(String::formatted("JS::Handle<JS::{}>", type.name())), .sequence_storage_type = SequenceStorageType::MarkedVector };
    ...
    if (type.name() == "sequence") {
        ...
        return { .name = MUST(String::formatted("{}<{}>", storage_type_name, sequence_cpp_type.name)), .sequence_storage_type = SequenceStorageType::Vector };
    }
    ...
    if (type.name() == "record") {
        ...
        return { .name = MUST(String::formatted("OrderedHashMap<{}, {}>", record_key_cpp_type.name, record_value_cpp_type.name)), .sequence_storage_type = SequenceStorageType::Vector };
    }
...
{
    if (input.is_one_of("class"_string, "template"_string, "for"_string, "default"_string, "char"_string, "namespace"_string, "delete"_string, "inline"_string, "register"_string)) {
        StringBuilder builder;
...
static void generate_include_for(auto& generator, auto& path)
{
    ...
    forked_generator.set("include.path", MUST(String::formatted("{}/{}.h", include_path.dirname(), include_path.title())));
...
static void emit_includes_for_all_imports(auto& interface, auto& generator, bool is_iterator = false)
{
    ...
    if (is_iterator) {
        auto iterator_path = MUST(String::formatted("{}Iterator", interface.fully_qualified_name.replace("::"sv, "/"sv, ReplaceMode::All)));
        generate_include_for_iterator(generator, iterator_path);
    }
}

template<typename ParameterType>
static void generate_to_string(SourceGenerator& scoped_generator, ParameterType const& parameter, bool variadic, bool optional, Optional<String> const& optional_default_value)
{
...
template<typename ParameterType>
static void generate_to_integral(SourceGenerator& scoped_generator, ParameterType const& parameter, bool optional, Optional<String> const& optional_default_value)
{
    struct TypeMap {
        StringView idl_type;
        StringView cpp_type;
...
static void generate_to_cpp(SourceGenerator& generator, ParameterType& parameter, String const& js_name, String const& js_suffix, String const& cpp_name, IDL::Interface const& interface, bool legacy_null_to_empty_string = false, bool optional = false, Optional<String> optional_default_value = {}, bool variadic = false, size_t recursion_depth = 0, bool string_to_fly_string = false)
{
    ...
    if (!@js_name@@js_suffix@.is_undefined())
        ...
        enum_generator.set("js_name.as_string", MUST(String::formatted("{}{}_string", enum_generator.get("js_name"sv), enum_generator.get("js_suffix"sv))));
    ...
    @parameter.type.name@ @cpp_name@ {};
        ...
        while (true) {
            for (auto& member : current_dictionary->members) {
                ...
                auto member_value_name = MUST(String::formatted("{}_value_{}", member_js_name, i));
                auto member_property_value_name = MUST(String::formatted("{}_property_value_{}", member_js_name, i));
    ...
    if (!@member_property_value_name@.is_undefined()) {
                ...
                generate_to_cpp(dictionary_generator, member, member_property_value_name, ""_string, member_value_name, interface, member.extended_attributes.contains("LegacyNullToEmptyString"), !member.required, member.default_value);
    ...
    if (!iterator_method@recursion_depth@)
        ...
        parameterized_type.generate_sequence_from_iterable(sequence_generator, MUST(String::formatted("{}{}", acceptable_cpp_name, optional ? "_non_optional" : "")), MUST(String::formatted("{}{}", js_name, js_suffix)), MUST(String::formatted("iterator_method{}", recursion_depth)), interface, recursion_depth + 1);
    ...
    for (auto& key@recursion_depth@ : record_keys@recursion_depth@) {
        ...
        generate_to_cpp(record_generator, key_parameter, "key"_string, String::number(recursion_depth), MUST(String::formatted("typed_key{}", recursion_depth)), interface, false, false, {}, false, recursion_depth + 1);
        ...
        generate_to_cpp(record_generator, value_parameter, "value"_string, String::number(recursion_depth), MUST(String::formatted("typed_value{}", recursion_depth)), interface, false, false, {}, false, recursion_depth + 1);
    ...
    auto @js_name@@js_suffix@_to_dictionary = [&vm, &realm](JS::Value @js_name@@js_suffix@) -> JS::ThrowCompletionOr<@dictionary.type@> {
        ...
        if (dictionary_type)
            to_variant_captures.append(MUST(String::formatted(", &{}{}_to_dictionary", js_name, js_suffix)));

        union_generator.set("to_variant_captures", MUST(to_variant_captures.to_string()));
    ...
    auto @js_name@@js_suffix@_to_variant = [@to_variant_captures@](JS::Value @js_name@@js_suffix@) -> JS::ThrowCompletionOr<@union_type@> {
        ...
        if (method) {
            ...
            sequence_type->generate_sequence_from_iterable(union_generator, acceptable_cpp_name, MUST(String::formatted("{}{}", js_name, js_suffix)), "method", interface, recursion_depth + 1);
        ...
        if (@js_name@@js_suffix@.is_number()) {
            ...
            IDL::Parameter parameter { .type = *numeric_type, .name = ""_string, .optional_default_value = {}, .extended_attributes = {} };
            generate_to_cpp(union_generator, parameter, js_name, js_suffix, MUST(String::formatted("{}{}_number", js_name, js_suffix)), interface, false, false, {}, false, recursion_depth + 1);
        ...
        if (string_type) {
            ...
            IDL::Parameter parameter { .type = *string_type, .name = ""_string, .optional_default_value = {}, .extended_attributes = {} };
            generate_to_cpp(union_generator, parameter, js_name, js_suffix, MUST(String::formatted("{}{}_string", js_name, js_suffix)), interface, false, false, {}, false, recursion_depth + 1);
        ...
        VERIFY(x.is_number());
            ...
            // NOTE: generate_to_cpp will use to_{u32,etc.} which uses to_number internally and will thus use TRY, but it cannot throw as we know we are dealing with a number.
            IDL::Parameter parameter { .type = *numeric_type, .name = ""_string, .optional_default_value = {}, .extended_attributes = {} };
            generate_to_cpp(union_numeric_type_generator, parameter, "x", ""_string, "x_number", interface, false, false, {}, false, recursion_depth + 1);
...
static void generate_argument_count_check(SourceGenerator& generator, String const& function_name, size_t argument_count)
{
    ...
    if (argument_count == 1) {
        ...
        argument_count_check_generator.set(".arg_count_suffix", MUST(String::formatted(", \"{}\"", argument_count)));
    }
...
static void generate_arguments(SourceGenerator& generator, Vector<IDL::Parameter> const& parameters, StringBuilder& arguments_builder, IDL::Interface const& interface)
{
    ...
    for (auto& parameter : parameters) {
        ...
        if (parameter.variadic) {
            ...
            parameter_names.append(MUST(String::formatted("move({})", parameter_name)));
        } else {
            ...
            if (parameter.extended_attributes.contains("ExplicitNull")) {
                arguments_generator.set("argument.size", String::number(argument_index + 1));
    ...
    auto arg@argument.index@ = vm.argument(@argument.index@);
        ...
        generate_to_cpp(generator, parameter, "arg"_string, String::number(argument_index), parameter.name.to_snakecase(), interface, legacy_null_to_empty_string, parameter.optional, parameter.optional_default_value, parameter.variadic, 0, fly_string);
        ++argument_index;
    }
...
void IDL::ParameterizedType::generate_sequence_from_iterable(SourceGenerator& generator, String const& cpp_name, String const& iterable_cpp_name, String const& iterator_method_cpp_name, IDL::Interface const& interface, size_t recursion_depth) const
{
    ...
    generate_to_cpp(sequence_generator, parameter, "next_item"_string, String::number(recursion_depth), MUST(String::formatted("sequence_item{}", recursion_depth)), interface, false, false, {}, false, recursion_depth);
...
static void generate_wrap_statement(SourceGenerator& generator, String const& value, IDL::Type const& type, IDL::Interface const& interface, StringView result_expression, WrappingReference wrapping_reference = WrappingReference::No, size_t recursion_depth = 0)
{
    ...
    if (!libweb_interface_namespaces.span().contains_slow(type.name())) {
        if (is_javascript_builtin(type))
            scoped_generator.set("type", MUST(String::formatted("JS::{}", type.name())));
        else
        ...
        builder.append("::"sv);
        builder.append(type.name());
        scoped_generator.set("type", MUST(builder.to_string()));
    }
    ...
    for (size_t i@recursion_depth@ = 0; i@recursion_depth@ < @value@_non_optional.size(); ++i@recursion_depth@) {
        ...
        if (is_platform_object(sequence_generic_type.parameters().first())) {
            ...
            generate_wrap_statement(scoped_generator, MUST(String::formatted("element{}", recursion_depth)), sequence_generic_type.parameters().first(), interface, MUST(String::formatted("auto wrapped_element{} =", recursion_depth)), WrappingReference::Yes, recursion_depth + 1);
        }
    ...
    @result_expression@ @value@.visit(
        ...
        [&vm, &realm]([[maybe_unused]] @current_type@ const& visited_union_value@recursion_depth@) -> JS::Value {
            ...
            generate_wrap_statement(union_generator, MUST(String::formatted("visited_union_value{}", recursion_depth)), current_union_type, interface, "return"sv, WrappingReference::No, recursion_depth + 1);
        ...
        if (type.is_nullable())
            scoped_generator.set("value", MUST(String::formatted("{}.value()", value)));
        scoped_generator.append(R"~~~(
  ...
  @result_expression@ @value@->callback;
    ...
    auto dictionary_object@recursion_depth@ = JS::Object::create(realm, realm.intrinsics().object_prototype());
        ...
        while (true) {
            for (auto& member : current_dictionary->members) {
                dictionary_generator.set("member_key", member.name);
                auto member_key_js_name = MUST(String::formatted("{}{}", make_input_acceptable_cpp(member.name.to_snakecase()), recursion_depth));
                dictionary_generator.set("member_name", member_key_js_name);
                auto member_value_js_name = MUST(String::formatted("{}_value", member_key_js_name));
                dictionary_generator.set("member_value", member_value_js_name);

                auto wrapped_value_name = MUST(String::formatted("wrapped_{}", member_value_js_name));
                dictionary_generator.set("wrapped_value_name", wrapped_value_name);

                dictionary_generator.append(R"~~~(
    JS::Value @wrapped_value_name@;
)~~~");
                generate_wrap_statement(dictionary_generator, MUST(String::formatted("{}.{}", value, member.name.to_snakecase())), member.type, interface, MUST(String::formatted("{} =", wrapped_value_name)), WrappingReference::No, recursion_depth + 1);
...
static void generate_variable_statement(SourceGenerator& generator, String const& variable_name, IDL::Type const& value_type, String const& value_name, IDL::Interface const& interface)
{
    ...
    return generate_wrap_statement(generator, value_name, value_type, interface, MUST(String::formatted("{} = ", variable_name)));
}
...
static void generate_function(SourceGenerator& generator, IDL::Function const& function, StaticFunction is_static_function, String const& class_name, String const& interface_fully_qualified_name, IDL::Interface const& interface)
{
    ...
    function_generator.set("overload_suffix", function.is_overloaded ? String::number(function.overload_index) : ""_string);
...
JS_DEFINE_NATIVE_FUNCTION(@class_name@::@function.name:snakecase@@overload_suffix@)
{
    ...
    [[maybe_unused]] auto retval = retval_or_exception.release_value();
        ...
        if (arguments_builder.is_empty())
            ...
            function_generator.set(".arguments", MUST(String::formatted("vm, {}", arguments_builder.string_view())));
...
static String generate_constructor_for_idl_type(Type const& type)
{
    ...
    case Type::Kind::Plain:
        return MUST(String::formatted("make_ref_counted<IDL::Type>(\"{}\", {})", type.name(), type.is_nullable()));
...
static void generate_dictionary_types(SourceGenerator& generator, Vector<String> const& dictionary_types)
{
    generator.append(R"~~~(
...
static void generate_overload_arbiter(SourceGenerator& generator, auto const& overload_set, IDL::Interface const& interface, String const& class_name, IsConstructor is_constructor)
{
    auto function_generator = generator.fork();
    ...
    HashTable<String> dictionary_types;
...
JS_DEFINE_NATIVE_FUNCTION(@class_name@::@function.name:snakecase@)
{
    ...
    for (auto argument_count = 0u; argument_count <= maximum_argument_count; ++argument_count) {
        ...
        function_generator.set("overload_count", String::number(effective_overload_set.size()));
    ...
    case @current_argument_count@: {
        ...
        for (auto& overload : effective_overload_set) {
            ...
            function_generator.set("overload.types", MUST(types_builder.to_string()));
            function_generator.set("overload.optionality_values", MUST(optionality_builder.to_string()));

            function_generator.appendln("        overloads.empend(@overload.callable_id@, @overload.types@, @overload.optionality_values@);");
        }

        function_generator.set("overload_set.distinguishing_argument_index", String::number(distinguishing_argument_index));
        function_generator.append(R"~~~(
...
static void generate_constructor(SourceGenerator& generator, IDL::Constructor const& constructor, IDL::Interface const& interface, bool is_html_constructor)
{
    ...
    constructor_generator.set("overload_suffix", constructor.is_overloaded ? String::number(constructor.overload_index) : ""_string);
...
static void generate_constructors(SourceGenerator& generator, IDL::Interface const& interface)
{
    ...
    generator.set("constructor.length", String::number(shortest_length));
...
static void generate_enumerations(HashMap<String, Enumeration> const& enumerations, StringBuilder& builder)
{
...
static void collect_attribute_values_of_an_inheritance_stack(SourceGenerator& function_generator, Vector<Interface const&> const& inheritance_chain)
{
    ...
    for (auto const& interface_in_chain : inheritance_chain.in_reverse()) {
        ...
        for (auto& attribute : interface_in_chain.attributes) {
            ...
            auto return_value_name = MUST(String::formatted("{}_retval", attribute.name.to_snakecase()));
    ...
    auto @attribute.return_value_name@ = TRY(throw_dom_exception_if_needed(vm, [&] { return impl->@attribute.cpp_name@(); }));
            ...
            generate_wrap_statement(attribute_generator, return_value_name, attribute.type, interface_in_chain, MUST(String::formatted("auto {}_wrapped =", return_value_name)));
    ...
    MUST(result->create_data_property("@attribute.name@", @attribute.return_value_name@_wrapped));
        ...
        for (auto& constant : interface_in_chain.constants) {
            ...
            generate_wrap_statement(constant_generator, constant.value, constant.type, interface_in_chain, MUST(String::formatted("auto constant_{}_value =", constant.name)));
...
static void generate_default_to_json_function(SourceGenerator& generator, String const& class_name, IDL::Interface const& start_interface)
{
...
static void generate_named_properties_object_declarations(IDL::Interface const& interface, StringBuilder& builder)
{
    ...
    generator.set("named_properties_class", MUST(String::formatted("{}Properties", interface.name)));
...
JS::ThrowCompletionOr<Optional<JS::PropertyDescriptor>> @named_properties_class@::internal_get_own_property(JS::PropertyKey const& property_name) const
{
    ...
    if (TRY(object.is_named_property_exposed_on_object(property_name))) {
        auto property_name_string = MUST(property_name.to_string());
...
static void generate_prototype_or_global_mixin_definitions(IDL::Interface const& interface, StringBuilder& builder)
{
    ...
    if (interface.pair_iterator_types.has_value()) {
        generator.set("iterator_name", MUST(String::formatted("{}Iterator", interface.name)));
    }
...
void @class_name@::initialize(JS::Realm& realm)
{
    ...
    for (auto& constant : interface.constants) {
        ...
        generate_wrap_statement(constant_generator, constant.value, constant.type, interface, MUST(String::formatted("auto constant_{}_value =", constant.name)));
    ...
    for (auto const& overload_set : interface.overload_sets) {
        ...
        function_generator.set("function.name:snakecase", make_input_acceptable_cpp(overload_set.key.to_snakecase()));
        function_generator.set("function.length", MUST(String::number(get_shortest_function_length(overload_set.value))));
...
JS_DEFINE_NATIVE_FUNCTION(@class_name@::@attribute.setter_callback@)
{
    ...
    reactions_stack.element_queue_stack.append({});
            ...
            generate_to_cpp(generator, attribute, "value"_string, ""_string, "cpp_value"_string, interface, attribute.extended_attributes.contains("LegacyNullToEmptyString"));
...
private:
    ...
    for (auto const& overload_set : interface.constructor_overload_sets) {
        ...
        if (overload_set.value.size() > 1) {
            for (auto i = 0u; i < overload_set.value.size(); ++i) {
                constructor_generator.set("overload_suffix", String::number(i));
                constructor_generator.append(R"~~~(
...
static void define_the_operations(SourceGenerator& generator, HashMap<String, Vector<Function&>> const& operations)
{
    for (auto const& operation : operations) {
        ...
        function_generator.set("function.length", String::number(get_shortest_function_length(operation.value)));
...
void generate_iterator_prototype_header(IDL::Interface const& interface, StringBuilder& builder)
{
    ...
    generator.set("prototype_class", MUST(String::formatted("{}IteratorPrototype", interface.name)));
...
void generate_iterator_prototype_implementation(IDL::Interface const& interface, StringBuilder& builder)
{
    ...
    generator.set("name", MUST(String::formatted("{}Iterator", interface.name)));
    generator.set("parent_name", interface.parent_name);
    generator.set("prototype_class", MUST(String::formatted("{}IteratorPrototype", interface.name)));
    generator.set("prototype_base_class", interface.prototype_base_class);
    generator.set("fully_qualified_name", MUST(String::formatted("{}Iterator", interface.fully_qualified_name)));
    generator.set("possible_include_path", MUST(String::formatted("{}Iterator", interface.name.replace("::"sv, "/"sv, ReplaceMode::All))));
