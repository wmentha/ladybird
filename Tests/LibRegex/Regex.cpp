...
TEST_CASE(catch_all_newline_view)
{
    ...
    EXPECT_EQ(result.success, true);
    EXPECT_EQ(result.count, 3u);
    String str = "Hello World"_string;
...
TEST_CASE(ini_file_entries)
{
    ...
    if constexpr (REGEX_DEBUG) {
        for (auto& v : result.matches)
            fprintf(stderr, "%s\n", MUST(v.view.to_string().characters()));
    }
...
TEST_CASE(ini_file_entries2)
{
    ...
    String haystack = "ViewMode=Icon"_string;
...
auto parse_test_case_long_disjunction_chain = MUST(String::repeated("a|"_string, 100000));
...
TEST_CASE(ECMA262_unicode_match)
{
    ...
    auto space_and_line_terminators = MUST(builder.to_string());
...
{
    Regex<ECMA262> re("(?:aa)*");
    auto result = re.match(MUST(String::repeated('a', 1000)));
    EXPECT_EQ(result.success, true);
}
...
static auto g_lots_of_a_s = MUST(String::repeated('a', 10'000'000));
...
{
    {
        ...
        EXPECT_EQ(MUST(result.matches.first().view.to_string()), "A"sv);
    }
}
...
{
    {
        ...
        EXPECT_EQ(result.success, true);
        EXPECT_EQ(result.matches.size(), 1u);
        EXPECT_EQ(MUST(result.matches.first().view.to_string()), ""sv);
    }
}
...
{
    {
        ...
        EXPECT_EQ(result.success, true);
        EXPECT_EQ(result.matches.size(), 1u);
        EXPECT_EQ(MUST(result.matches.first().view.to_string()), "h"sv);
    }
    {
        ...
        EXPECT_EQ(result.success, true);
        EXPECT_EQ(result.matches.size(), 1u);
        EXPECT_EQ(MUST(result.matches.first().view.to_string()), "slideNumbers"sv);
        EXPECT_EQ(MUST(result.capture_group_matches.first()[0].view.to_string()), "slideNumbers"sv);
        EXPECT_EQ(MUST(result.capture_group_matches.first()[1].view.to_string()), "}"sv);
    }
    {
        // #21786, /[^\S\n]/.exec("\n") should be null, not [ "\n" ].
